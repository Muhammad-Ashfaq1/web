...
class FileUploadManager
{
    public static function uploadFile(UploadedFile $file, $file_path = null)
    {
        ...
        // Write to the public disk explicitly with public visibility so the file is world-readable (0644).
        // Without this we fall through to the default ("local") disk which writes 0600 — unreadable by the
        // web-server user on shared hosting (LiteSpeed/Apache running as `nobody`), causing 403s on assets.
        if (Storage::disk('public')->put($file_path.$file_path_name, file_get_contents($file), 'public')) {
